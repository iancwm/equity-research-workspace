...
- existing ledger, assumptions, dependency graph, and source registry;
...
If existing state is present, use [selective recomputation](workflow-recompute.md) to decide which nodes are stale. Do not rerun stable nodes merely because a new report is being generated.

## Step 6 — Validate before publication

Run both:

- `<skill-root>/scripts/validate_workspace.py` against the workspace;
- the [analytical QA review](qa-review.md).
